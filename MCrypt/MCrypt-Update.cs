...
namespace MCrypt
{
    public partial class MCrypt_Update : Form
    {
        Core core = new Core();
        ...
        private string getLatestVersion()
        {
            try
            {
                ...
                byte[] html = client.DownloadData("http://builds.mullak99.co.uk/MCrypt/checkupdate.php?branch=" + Program.getBranch());
        ...
        public static void selfUpdate(bool doCleanUpdate = false)
        {
            ...
            if (checkServerConnection())
                try
                {
                    File.Delete(Path.Combine(installDir, "MCryptUpdater.exe"));
                    File.Delete(Path.Combine(installDir, "updater.pack"));
                    WebClient webClient = new WebClient();
                    webClient.DownloadFile(new Uri("http://builds.mullak99.co.uk/MCrypt/updater/latest"), Path.Combine(installDir, "updater.pack"));
                    ...
                    args += "--silent ";
                    Process.Start(Path.Combine(installDir, "MCryptUpdater.exe"), args);

                    Environment.Exit(0);
                }
                catch (UnauthorizedAccessException)
                {
                    if (MessageBox.Show("You are not running MCrypt as an admin, by doing this you cannot update the application in admin protected directories.\n\nDo you want to launch as admin?", "Notice", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        runAsAdmin();
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString(), "Error");
                    if (File.Exists(Path.Combine(installDir, "MCryptUpdater.exe")))
                        File.Delete(Path.Combine(installDir, "MCryptUpdater.exe"));
